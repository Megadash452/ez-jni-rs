...
use crate::{call::Env, types::{Class, SigType}};

/// Generates code that brings into *scope* the necessary **traits and locals** (a.k.a a *prelude*) to call the functions in the code generated by the macros.
/// 
/// This will also generate an **`env` local** if necessary.
/// The only places where the `env` is not generated is in [`jni_fn`][crate::jni_fn::jni_fn()] and the [derive macros][crate::object]
/// (see [`Env::Argument`]).
