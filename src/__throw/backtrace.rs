use std::{backtrace::{Backtrace as StdBacktrace, BacktraceStatus}, borrow::Borrow, error::Error, fmt::Display, str::FromStr};
...
impl Backtrace {
    /// Create a [`Backtrace`] from information captured by the *panic hook*.
    pub fn new_from_captured(backtrace: StdBacktrace) -> Option<Self> {
        if backtrace.status() == BacktraceStatus::Captured {
            Self::parse(backtrace).ok()
        } else {
            None
        }
    }
    /// Same as [new_from_captured][Backtrace::new_from_captured()], but always returns a [`Backtrace`].
    /// 
    /// This should only be used when running in an *integration test*.
    /// To display as much detail as possible, the Backtrace is forcibly captured,
    /// and even if that failed, this will inject fake Backtrace frames with the current known panic location.
    #[doc(hidden)]
    #[inline(always)]
    pub fn new_from_captured_integration_test(backtrace: StdBacktrace, location: Location) -> Self {
        Self::new_from_captured(backtrace)
    ...
    fn new_unknown(location: Location) -> Self {
    ...
    pub(super) fn append_stacktrace(&mut self, exception: &JThrowable<'_>, env: &mut JNIEnv<'_>) {
        ...
        stacktrace.extend(backtrace.into_iter());
        *self = Self(stacktrace);
    }
    ...
    #[inline(always)]
    fn parse(backtrace: StdBacktrace) -> Result<Self, ParseBacktraceError> {
        parse_rust_backtrace(&backtrace.to_string())
    }
}
impl FromStr for Backtrace {
    type Err = ParseBacktraceError;

    #[inline(always)]
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        parse_rust_backtrace(s)
    }
}
impl Display for Backtrace {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for (i, frame) in self.frames().iter().enumerate() {
            writeln!(f, "   {i}: {symbol}\n             at {loc}", symbol=frame.symbol, loc=frame.location)?;
        }
        Ok(())
    }
}
impl AsRef<[BacktraceElement]> for Backtrace {
    #[inline(always)]
    fn as_ref(&self) -> &[BacktraceElement] {
        &self.0
    }
}
impl Borrow<[BacktraceElement]> for Backtrace {
    #[inline(always)]
    fn borrow(&self) -> &[BacktraceElement] {
        &self.0
    }
}
...
pub struct BacktraceElement {
    /// The name/path of the **function** of a [`BacktraceElement`].
    symbol: String,
    location: Location,
}

#[derive(Debug, Error)]
pub enum ParseBacktraceError {
...
fn parse_rust_backtrace(backtrace: &str) -> Result<Backtrace, ParseBacktraceError> {
    ...
    let filtered = parse_backtrace_frames(backtrace)?
        .into_iter()
        .filter(|(symbol, _)| !{
...
/// TODO: test inject_backtrace() and get_java_stacktrace() with specific frames
...
pub fn inject_backtrace(exception: &JThrowable<'_>, backtrace: &[BacktraceElement], env: &mut JNIEnv<'_>) {
    ...
    let entry_frame = stacktrace.remove(index);
    ...
    stacktrace.splice(index..index, rust_trace);

    // Set Exception's StackTrace
    call!(env=> exception.setStackTrace([java.lang.StackTraceElement](stacktrace)) -> void);
}

fn get_java_stacktrace(exception: &JThrowable<'_>, env: &mut JNIEnv<'_>) -> Box<[BacktraceElement]> {
    call!(env=> exception.getStackTrace() -> [java.lang.StackTraceElement])
        .into_iter()
        .map(|element| {
            let class = call!(env=> element.getClassName() -> String);
            let method = call!(env=> element.getMethodName() -> String);
            let location = Location {
                file: call!(env=> element.getFileName() -> String),
                line: call!(env=> element.getLineNumber() -> int) as u32, // Safe: line number can't be negative
                col: 0 // StackTraceElement does not keep track of column number
            };
            BacktraceElement { location, symbol: format!("{class}.{method}") }
        })
        .collect()
}
