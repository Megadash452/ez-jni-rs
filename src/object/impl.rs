...
use crate::{utils::{check_object_class, create_array_object, get_array_object, JniResultExt}, Class, __throw::get_jni_error_msg};
use super::*;
...
impl FromObject<'_, '_, '_> for GlobalRef {
...
where T: ToObject + Class {
    #[inline(always)]
    fn to_object_env<'local>(&self, env: &mut JNIEnv<'local>) -> JObject<'local> {
        create_array_object(self, &T::class(), env)
    }
}
impl<T> ToObject for &[T]
where T: ToObject + Class {
...
impl ToObject for i8 {
    fn to_object_env<'local>(&self, env: &mut JNIEnv<'local>) -> JObject<'local> {
        // FIXME: replace constructor with .valueOf() because constructor is deprecated. Do this for all primitives
